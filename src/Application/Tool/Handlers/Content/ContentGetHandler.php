...
final class ContentGetHandler implements ToolHandlerInterface
{
    ...
    public function __construct(ContentToolService $tools)
    {
        ...
        $this->contract = new ToolContract(
            'content_get',
            ToolPromptDescriptions::for('content_get'),
            ToolSchemas::closedObject(array(
                'content_ref' => ToolSchemas::opaqueRef('d'),
            ), array('content_ref'))
        );
    }
    ...
    {
        return $this->tools->get((string) $arguments['content_ref'], $context);
    }
}
